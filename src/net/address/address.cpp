...
static constexpr auto is_binary = [](std::span<const char> data) noexcept {
    return std::ranges::any_of(data, [](const char c) { return !std::isprint(static_cast<unsigned char>(c)); });
};

static constexpr auto to_hex = [](std::span<const std::byte> data) {
    static const std::string up = "0123456789ABCDEF";
...
static constexpr auto is_supported_family = [](sa_family_t family) noexcept {
...
bool zportal::SockAddress::is_connectable() const noexcept {
    ...
    if (family() == AF_INET) {
        const auto* ip4 = reinterpret_cast<const sockaddr_in*>(&ss_);
        return ip4->sin_addr.s_addr != 0 && ip4->sin_port != 0;
    }
    if (family() == AF_INET6) {
        const auto* ip6 = reinterpret_cast<const sockaddr_in6*>(&ss_);
        return !IN6_IS_ADDR_UNSPECIFIED(&ip6->sin6_addr) && !IN6_IS_ADDR_MULTICAST(&ip6->sin6_addr) &&
               ip6->sin6_port != 0 && (!IN6_IS_ADDR_LINKLOCAL(&ip6->sin6_addr) || ip6->sin6_scope_id != 0);
    }
    if (family() == AF_UNIX) {
        return true;
    }
...
zportal::SockAddress zportal::SockAddress::from_sockaddr(const sockaddr* addr, socklen_t len) {
    ...
    if (addr->sa_family == AF_INET6 && len != sizeof(sockaddr_in6)) {
        throw std::invalid_argument("invalid socklen");
    }
...
zportal::SockAddress zportal::SockAddress::ip4_numeric(const std::string& numeric, std::uint16_t port) {
    ...
    const int result = ::inet_pton(AF_INET, numeric.c_str(), &ip4->sin_addr);
    if (result == -1) {
