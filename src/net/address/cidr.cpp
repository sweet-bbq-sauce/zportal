...
std::uint8_t zportal::Cidr::get_prefix() const noexcept {
    return prefix_;
}
...
zportal::Cidr zportal::Cidr::get_network() const {
    ...
    if (sa->sa_family == AF_INET6) {
        ...
        auto* b = reinterpret_cast<std::uint8_t*>(&sin6.sin6_addr);

        std::uint8_t bits = prefix_;
        std::size_t full_bytes = bits / 8;
        auto rem_bits = static_cast<std::uint8_t>(bits % 8);

        for (std::size_t i = full_bytes + ((rem_bits != 0U) ? 1 : 0); i < 16; ++i) {
            b[i] = 0;
        }

        if (rem_bits != 0 && full_bytes < 16) {
            auto m = static_cast<std::uint8_t>(0xFFU << (8U - rem_bits));
...
bool zportal::Cidr::is_in_network(const SockAddress& address) const noexcept {
    ...
    if ((net_sa == nullptr) || (addr_sa == nullptr)) {
        return false;
    }
    ...
    if (net_sa->sa_family == AF_INET6) {
        ...
        const auto* nb = reinterpret_cast<const std::uint8_t*>(&net6.sin6_addr);
        const auto* ab = reinterpret_cast<const std::uint8_t*>(&a6.sin6_addr);
